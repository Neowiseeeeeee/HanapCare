      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-6 right-6 z-50 w-[360px] max-w-[calc(100vw-24px)] bg-white rounded-2xl shadow-2xl shadow-black/20 flex flex-col overflow-hidden border border-slate-200"
            style={{ height: "520px" }}
          >
            {/* Header */}
            <div className="bg-gradient-to-r from-sky-600 to-teal-600 px-4 py-3.5 flex items-center gap-3">
                ...
                <div className="flex flex-wrap gap-2 pt-1">
                  {QUICK_QUESTIONS.map((q) => (
                    <button
                      ...
                      className="px-3 py-1.5 bg-white border border-sky-200 text-sky-700 text-xs font-medium rounded-full hover:bg-sky-50 hover:border-sky-400 transition-colors shadow-sm"
                    >
                      {q}
                    </button>
                  ))}
